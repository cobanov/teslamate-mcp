...
def validate_sql_query(sql: str) -> tuple[bool, Optional[str]]:
    ...
    forbidden_pattern = r'\b(DROP|CREATE|INSERT|UPDATE|DELETE|ALTER|TRUNCATE|GRANT|REVOKE)\b'
    if re.search(forbidden_pattern, sql_no_strings, re.IGNORECASE):
        return False, f"Forbidden SQL operation detected. Only SELECT queries are allowed."
...
def main(
    ...
    async def call_tool(name: str, arguments: dict) -> list[types.ContentBlock]:
        ...
        return [
            types.TextContent(
                type="text",
                text=json.dumps(result, indent=2, default=str)  # Convert to JSON string
            )
        ]
    ...
    async def get_efficiency_by_month_and_temperature() -> List[Dict[str, Any]]:
        return await execute_sql_query("queries/efficiency_by_month_and_temperature.sql")
